...
DateTime _startOfWeek(DateTime date) {
  ...
  return utcDate.subtract(Duration(days: utcDate.weekday - 1)); // Monday is 1, Sunday is 7
}
...
int _calculateDailyStreak(String taskId, List<TaskCompletion> allCompletions) {
  final taskCompletions = allCompletions
      ...
      .toSet()
      .toList()..sort((a, b) => b.compareTo(a)); // Descending
...
int _calculateWeeklyStreak(String taskId, List<TaskCompletion> allCompletions) {
  final taskCompletionsOnFirstDayOfWeek = allCompletions
      .where((c) => c.taskId == taskId)
      .map((c) => _startOfWeek(c.date))
      .toSet()
      .toList()..sort((a, b) => b.compareTo(a)); // Descending
  ...
  DateTime startOfLastWeek = startOfCurrentWeek.subtract(const Duration(days: 7));

  if (taskCompletionsOnFirstDayOfWeek.contains(startOfCurrentWeek)) {
    currentStreak = 1;
    DateTime weekToCheck = startOfLastWeek;
    int currentWeekIndex = taskCompletionsOnFirstDayOfWeek.indexOf(startOfCurrentWeek);
    for (int i = currentWeekIndex + 1; i < taskCompletionsOnFirstDayOfWeek.length; i++) {
    ...
    int lastWeekIndex = taskCompletionsOnFirstDayOfWeek.indexOf(startOfLastWeek);
    for (int i = lastWeekIndex + 1; i < taskCompletionsOnFirstDayOfWeek.length; i++) {
       if (taskCompletionsOnFirstDayOfWeek[i] == weekToCheck) {
        currentStreak++;
        weekToCheck = weekToCheck.subtract(const Duration(days: 7));
      } else if (taskCompletionsOnFirstDayOfWeek[i].isBefore(weekToCheck)) {
